...
/* The following code merges individuals in the Social Register with individuals in the census of the same year using increasing geographic progressivity and using restriction on the first name, last name, middle name initial and first name of the sposue in ascending order (from less to more restrictive).
...
Section 3 cleans datsets of matched individuals from duplicates.

Section 4 creates final datasets of duplicates, matched and unmatched individuals.

Section 5 attaches census address to each individual. */
...
global temporary     "~/Dropbox/register_trial"                         // Location where temporary files are located (create your own temporary folder)


****************************************************************************
* SECTION 1. Merge register and census 1900 according to standardized name *
****************************************************************************

/*1.1.1 Merge individuals in other states according to first name and last name */
...
save $matchdir//Final/New_York_1900_stand.dta , replace                                     // Store results for 1:1 matched
...
drop if (stateicp==13 | stateicp==99)                              //No New York                                                                
merge 1:1 id using $matchdir//Final/New_York_1900_stand.dta, keepusing(l_name_cleaned f_name_cleaned) keep(master) nogenerate  // Remove the already matched individuals 
...
append using  $matchdir//Final/New_York_1900_stand.dta                                          // Store results for 1:1 matched
sort stateicp l_name_cleaned f_name_cleaned f_name_spouse_cleaned
save $matchdir//Final/New_York_1900_stand.dta   , replace 
...
/*1.1.4 Merge individuals in other states according to first name, last name, middle name initial and, first name of the spouse*/
...
merge 1:m stateicp l_name_cleaned f_name_cleaned m_name_initial f_name_spouse_cleaned using "$census/Other_States_1900_census_standardized.dta", keepusing(histid bpl age relate) keep(master matched)  // Merge according to last name cleaned, first name cleaned and middle name initial
...
/*1.2.1 Merge individuals in New York city according to first name and last name */
...
/*1.2.4 Merge individuals in New York city according to first name, last name, middle name initial and, first name of the spouse*/
...
/*1.3.1 Merge individuals in New York SEA according to first name and last name */
...
/*1.3.4 Merge individuals in New York SEA according to first name, last name, middle name initial and, first name of the spouse*/
...
/*1.4.1 Merge individuals in New York State according to first name and last name */
...
/*1.4.4 Merge individuals in New York State according to first name, last name, middle name initial and, first name of the spouse*/
...
/*2.1.1 Merge individuals in other states according to first name and last name */
use "$registers/Social Registers v3 inferred gender standardized states.dta", clear
keep if (city=="New York" & year_reg==1900)                                                //Register restriction
drop if (stateicp==13 | stateicp==99)                                                    //No New York
...
merge 1:m stateicp l_name_nysiis f_name_nysiis using "$census/Other_States_1900_census_standardized.dta", keepusing(histid bpl age relate) keep(master matched)  // Merge according to last and first name cleaned
...
save $matchdir//Final/New_York_1900_nysiis.dta , replace 
...
keep if (city=="New York" & year_reg==1900)    						//Restrictions   
drop if (stateicp==13 | stateicp==99)                              //No New York                                                                
merge 1:1 id using $matchdir//Final/New_York_1900_nysiis.dta, keepusing(l_name_cleaned f_name_cleaned) keep(master) nogenerate  // Remove the already matched individuals 
...
append using  $matchdir//Final/New_York_1900_nysiis.dta                                          // Store results for 1:1 matched
sort stateicp l_name_cleaned f_name_cleaned f_name_spouse_cleaned
save $matchdir//Final/New_York_1900_nysiis.dta   , replace 
...
merge 1:m stateicp l_name_nysiis f_name_nysiis m_name_initial f_name_spouse_nysiis using "$census/Other_States_1900_census_standardized.dta", keepusing(histid bpl age relate) keep(master matched)  // Merge according to last name cleaned, first name cleaned and middle name initial
...
/*2.2.1 Merge individuals in New York city according to first name and last name */
...
/*2.2.4 Merge individuals in New York city according to first name, last name, middle name initial and, first name of the spouse*/
...
/*2.3.1 Merge individuals in New York SEA according to first name and last name */
...
/*2.3.4 Merge individuals in New York SEA according to first name, last name, middle name initial and, first name of the spouse*/
...
/*2.4.1 Merge individuals in New York State according to first name and last name */
...
/*2.4.4 Merge individuals in New York State according to first name, last name, middle name initial and, first name of the spouse*/
...
append using  $matchdir//Final/New_York_1900_nysiis.dta  
gen technique="."
replace technique="NYSIIS"                                       // Store results for 1:1 matched
sort stateicp l_name_cleaned f_name_cleaned m_name_initial f_name_spouse_cleaned
save $matchdir//Final/New_York_1900_nysiis.dta , replace  
...
/* 3.1 Clean standardized dataset */

use $matchdir//Final/New_York_1900_stand.dta , clear
bys histid:  gen dup_histid = cond(_N==1,0,_n)  //Create a variable for histid duplicates
tab dup_histid
br if dup_histid>0   
...
preserve  //Save datset without duplicates
drop if dup_histid>0
br
save $matchdir//Final/New_York_1900_stand_temporary.dta, replace  
restore

preserve  //Save datset of duplicates
keep if dup_histid>0
br
save "$temporary/duplicates_histid_stand.dta", replace 
restore


/* 3.2 Clean NYSIIS dataset */

use $matchdir//Final/New_York_1900_nysiis.dta , clear
bys histid:  gen dup_histid = cond(_N==1,0,_n)  //Create a variable for histid duplicates
tab dup_histid
br if dup_histid>0
br id household_id household_structure title l_name first_name m_name f_name_spouse state_correct relate age bpl histid match technique dup_histid if dup_histid>0
...
drop if dup_histid>0
br
save $matchdir//Final/New_York_1900_nysiis_temporary.dta, replace  
restore

preserve   //Save datset of duplicates
keep if dup_histid>0
save "$temporary/duplicates_histid_nysiis.dta", replace 
restore

***************************************************
* SECTION 4. Final datasets of matched individuals *
***************************************************

/* 4.1 Create dataset of duplicates prioritizing standardized technique */

use "$temporary/duplicates_histid_stand.dta", clear
merge 1:1 id using "$temporary/register_trial/duplicates_histid_nysiis.dta", nogenerate
save "$temporary/duplicates_histid_final.dta", replace   


/* 4.2 Final cleaned dataset prioritizing standardized technique over pronunciation in case of multiple matches for the same register_id */

use $matchdir//Final/New_York_1900_stand_temporary.dta , clear
merge 1:1 id using $matchdir//Final/New_York_1900_nysiis_temporary.dta, nogenerate

bys histid:  gen dup_histid2 = cond(_N==1,0,_n)   //Create a variable for second type of histid duplicates
tab dup_histid2
*br id household_id household_structure title l_name first_name m_name f_name_spouse state_correct relate age bpl histid match technique dup_histid if dup_histid2>0 
drop if (dup_histid2>0 & technique=="NYSIIS")

save $matchdir//Final/New_York_1900_final_temporary.dta, replace


/* 4.3 Remove duplicates from final dataset */

use $matchdir//Final/New_York_1900_final_temporary.dta, clear
merge 1:1 id using "$temporary/duplicates_histid_final.dta", keep(master)
save $matchdir//Final/New_York_1900_final_temporary.dta, replace



/* 4.4 Create dataset of unmatched individuals */

use "$registers/Social Registers v3 inferred gender standardized states.dta", clear
keep if (city=="New York" & year_reg==1900) 
merge 1:1 id using $matchdir//Final/New_York_1900_final_temporary.dta, keepusing(l_name_cleaned f_name_cleaned) keep(master) nogenerate 
merge 1:1 id using "$temporary/duplicates_histid_final.dta", keepusing(l_name_cleaned f_name_cleaned) keep(master) nogenerate 
save "$temporary/New_York_1900_unmatched.dta", replace 



********************************************************
* SECTION 5. Add census address to matched individuals *
********************************************************
use $matchdir//Final/New_York_1900_final_temporary.dta, clear
merge 1:1 histid using "$ipums/streets/streets1900.dta", keepusing(street) keep(master matched)
save $matchdir//Final/New_York_1900_final_temporary.dta, replace
